...
input.addEventListener("keydown", (ev) => {
 ...
 if (ev.key === "Enter") {
   ...
   if (input == "") {
     options (displayButtons, input.value);
   } else {
...
const displayButtons = (keys) => {
 keys.forEach(target => {
   const ul = document.getElementById('available-targets');
...
const setOutput = (result) => {
 ...
 output.innerHTML = result.valediction + ", " + result.target;
 /*const ul = document.getElementById('available-targets');
 const newLi = document.createElement("li");
 newLi.textContent = result.valediction + ", " + result.target;
